...
import { Flex, Link, Button, Icon, Text, HStack, useColorMode, useColorModeValue } from '@chakra-ui/react';
...
export const Header = forwardRef(function Header(props, forRef) {
  ...
  return (
    ...
    >
      <HStack spacing='0' alignItems='center'>
        <Link key='logo' href='/' _hover={{ textDecoration: 'none' }} m='2' display='flex' alignItems='center' ml='50'>
          <Icon as={chdLogo} />
          <Text ml='2' mr='2'>
      ...
      <HStack as='nav' spacing='4' display={{ base: 'none', md: 'flex' }}>
        {props.linkArray.map((link) => (
          <Button key={link.name} onClick={() => handleScroll(link.linkRef)}>
            {link.name}
          </Button>
        ))}
        <Link
